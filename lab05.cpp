 ...
 *    James Olney
 * Summary:
 *    A program to multiply two matrices together.
 ************************************************************************/
...
/**************************************************
 * GET CELL PRODUCT
 * Calculates the product for a single cell
 *************************************************/
void * getCellProduct(void * p)
{
   Cell *cell = (Cell*)p;
   MatrixMultiplication* m = cell->m;
   int row = cell->row, 
   col = cell->col,
   sum = 0, 
   max = m->numColLHS, 
   numColLHS = m->numColLHS,
   numColRHS = m->numColRHS;
   
   for(int x = 0; x < max; ++x)
   {
      int lhs = m->arrayLHS[row * numColLHS + x],
	  rhs = m->arrayRHS[x * numColRHS + col],
	  product = lhs * rhs;
      sum += product; 
   }
   m->arrayDestination[row * numColRHS + col] = sum;
   pthread_exit(NULL);
}
...
bool multiply(MatrixMultiplication & m)
{
   ...
   if (m.numRowRHS != m.numColLHS)
      return false;
//my code
   pthread_t* threads = new pthread_t[numRowDes * numColDes];
   Cell* myCell = new Cell[numRowDes * numColDes];
   for(int row = 0; row < numRowDes; row++)
   {
      for(int col = 0; col < numColDes; col++)
      {
        myCell[row * numColDes + col].row = row;
		myCell[row * numColDes + col].col = col;
		myCell[row * numColDes + col].m = &m;
		pthread_create(&(threads[row * numColDes + col]),
                     NULL,
                     getCellProduct,
                     (void *)&myCell[row * numColDes + col]); 
      }
   }
   for(int x = 0; x < (numRowDes * numColDes); ++x)
...
void display(MatrixMultiplication & m)
{
   ...
   numRowDes = m.numRowLHS,
   numColDes = m.numColRHS;
