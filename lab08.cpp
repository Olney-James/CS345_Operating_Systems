...
bool readMatrix(const char * fileName, int * & array, int & numRow, int & numCol);
bool multiply(MatrixMultiplication & m, int threadCount);
...
int main(int argc, char ** argv){
   ...
   cin >> threadCount;
   // multiply
   if (!multiply(matrix, threadCount)){
...
bool multiply(MatrixMultiplication & m, int threadCount){
   ...
   if (m.numColLHS != m.numRowRHS){
      return false;
   }
   
   for (int i = 0; i < numRowDes * numColDes; i++){
      total = 0;
      colRHS = i % matrix.numColRHS;
      rowLHS = (i - colRHS) / matrix.numColRHS;
	  
      for (int x = rowLHS * matrix.numColLHS; x < rowLHS * matrix.numColLHS + matrix.numColLHS; x++){
         total += matrix.arrayLHS[x] * matrix.arrayRHS[colRHS];
         colRHS += matrix.numColRHS;
      }
      matrix.arrayDestination[i] = total;
   }     
   return true;
}
